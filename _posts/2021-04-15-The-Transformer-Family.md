...
我们将模型中第$\tau+1$-th个segment在第$n$-th层中的隐状态记作$\mathbf{h}_{\tau+1}^{(n)} \in \mathbb{R}^{L \times d}$。除了同一个segment  在上一层中的隐状态$\mathbf{h}_{\tau+1}^{(n-1)}$，它还依赖于上一个segment 在同一层中的隐状态$\mathbf{h}_{\tau}^{(n)}$。通过与这些先前的隐状态中的信息相结合，模型便可将attention跨度在多个segments上向更久远的时间延伸。
$$
\begin{aligned}
